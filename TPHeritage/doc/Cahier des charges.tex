...
\emph{Note : si une commande est mal formée, on renverra \texttt{ERR}}.
...
Description : 
Ajoute un rectangle définit par son point gauche-haut $(X_1,Y_1)$, et sont point droite-bas $(X_2,Y_2)$. L'objet à un nom [name] qui est un mot composé de lettres et ou chiffres sans séparateurs à l'intérieur. Renvoie OK si la commande s'est bien exécutée, ERR dans le cas contraire (Exemple paramètres invalides ou erreur mémoire). La réponse peut s’accompagner d'un commentaire (ligne qui commence avec un \#)
...
Description : 
Vérifie si le point de coordonnées X Y est à l'intérieur (y compris sur le périmètre avec $\pm 0.5$ unité de marge d'erreur sur l’appartenance au segment )
% TODO : Vérifier la marge d'erreur
de l'objet  Name. Renvoie YES si c'est le cas, sinon NO. La commande renvoie aussi NO si l'objet n'existe pas.
